...
const lastAttributeNameRegex =
	/([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F "'>=/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/ // eslint-disable-next-line no-control-regex
...
const isIterable = value => {
	return (
		Array.isArray(value) || // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const prepareTemplateStyles = (scopeName, renderedDOM, template) => {
	...
	const { length } = styles // If there are no styles, skip unnecessary work
...
class UpdatingElement extends HTMLElement {
	...
	_attributeToProperty(name, value) {
		...
		if (propName !== undefined) {
			...
			this[propName] = ctor._propertyValueFromAttribute(value, options) // tslint:disable-next-line:no-any // mark state not reflecting
		...
		if (Array.isArray(userStyles)) {
			...
			const addStyles = (styles, set) =>
				styles.reduceRight(
					(
						set,
						s // Note: On IE set.add() does not return the set
